...
export const CIPHER_DETAILS = {
  ...
  pigpen: {
    difficulty: '中等',
    howItWorks: '猪圈密码（Masonic/Pigpen）用四组几何格：两个 3×3 方格（无点/有点）、两个 X 形格（无点/有点），每格对应一个字母。密文区用猪圈字体显示几何符号，与下方标准对照图一致。',
    steps: ['对照下方字表找到字母所在格子', '密文区以猪圈字体显示对应边框形状', '解密时按符号查表还原字母', '仍兼容旧格坐标 A1/B2'],
    examplePlain: 'HELLO',
    exampleCipher: 'HELLO',
  ...
  semaphore: {
    difficulty: '入门',
    howItWorks: '每字母对应唯一的两面旗方向组合（本工具内置 26 组双射表，便于练习往返）。密码吧/CTF 视觉题常见。',
    steps: ['A–Z 映射到 9 种方向的两两组合', '加密输出 ↖↑↗←●→↙↓↘ 符号', '解密时按空格分组还原字母'],
    examplePlain: 'HI',
    seeAlso: ['tap-code', 'morse', 'nato'],
  },
  'rail-fence': {
    difficulty: '入门',
    howItWorks: '栅栏密码（Rail Fence）将明文按 W 形写入 n 行，再按行顺序读出密文。CTF 签到题高频。',
    steps: ['设定栏数 n', '明文 zigzag 写入 n 行', '按行拼接得密文', '解密为逆过程'],
    examplePlain: 'WEAREDISCOVERED',
    seeAlso: ['columnar', 'scytale'],
  },
  bacon: {
    difficulty: '入门',
    howItWorks: 'Bacon 用 a/b 双字符组表示 5 位二进制，再映射 A–Z。可隐藏于大小写、字体粗细中。',
    steps: ['每字母转 5 位', '5 位对应 a/b', '拼接后按 5 位分组还原字母'],
    examplePlain: 'HELLO',
    seeAlso: ['binary', 'a1z26'],
  },
  'keyboard-shift': {
    difficulty: '入门',
    howItWorks: 'QWERTY 键盘移位：明文每个字母替换为同行键位上偏移 n 格的键（密码吧「QWE 类」题）。',
    steps: ['确定偏移量（通常 1–3）', '在 qwerty/asdfgh/zxcvbn 各行内移位', '解密反向移位'],
    seeAlso: ['caesar', 'jcuken'],
  },
  vigenere: {
    difficulty: '中等',
